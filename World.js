...
function main() {
  ...
  cl.scale.set(2, 1, 1);
  cl.rotation.y = Math.PI * 0.75;

  const cabinlightcolor = 0xfccc62;
  const cabinlightintensity = 0.5;
  let cabinlight = new THREE.PointLight(
    cabinlightcolor,
    cabinlightintensity,
    5
  );
  ...
  let fireflyPosAttribute = new THREE.BufferAttribute(
    new Float32Array(firefliesPositions),
    3
  ); // 3 because a position is x,y,z we would use 2 for uvs
  firefliesGeometry.setAttribute("position", fireflyPosAttribute);
  let uniforms = {
    uTime: { value: 0 },
    uRandom: { value: Math.random() },
  };
  let firefliesMaterial = new THREE.ShaderMaterial({
    uniforms: uniforms,
  ...
  barnmtlLoader.load("Objects/Barn/1267 Farm.mtl", (mtl) => {
    ...
    barnobjLoader.load("Objects/Barn/1267 Farm.obj", (root) => {
      root.traverse(function (node) {
        if (node.isMesh) node.material = barnmaterial;
      });
      scene.add(root);
  ...
  bl.scale.set(0.7, 2.5, 2);
  bl.rotation.y = Math.PI * -0.4;
  ...
  //Reeds
  const reedradius = 0.3;
  const reedheight = 8;
  const reedradialSegments = 16;
  const reedgeometry = new THREE.ConeGeometry(
    reedradius,
    reedheight,
    reedradialSegments
  );
  ...
  for (let x = 0; x < 400; x += 2) {
    for (let z = 0; z < 400; z += 2) {
      ...
      if (!(-19 < newx && newx < 19 && -19 < newz && newz < 19)) {
        ...
        grassobj.scale.set(1, 0.5 + Math.random(), 1);
  ...
  function drawRock(i, j, rot) {
    if (!(-10 < i && i < 10 && -10 < j && j < 10)) {
      ...
      scene.add(rock);
      rock.position.set(i, 0.5, j);
      let size = Math.random();
      rock.scale.set(0.2 + size, 0.2, 0.1 + size);
      rock.rotation.set(0, 0, 0.1 * rot);
    }
  }
  ...
  saucermtlLoader.load("Objects/Saucer/1352 Flying Saucer.mtl", (mtl) => {
    ...
    saucerobjLoader.load("Objects/Saucer/1352 Flying Saucer.obj", (saucer) => {
      saucer.traverse(function (node) {
        if (node.isMesh) node.material = saucermaterial;
      });
      saucer.name = "saucer";
  ...
  const sl_geometry = new THREE.SphereGeometry(
    sl_radius,
    sl_widthSegments,
    sl_heightSegments
  );
  ...
  cowmtlLoader.load("Objects/Cow/Cow.mtl", (mtl) => {
    ...
    cowobjLoader.load("Objects/Cow/Cow.obj", (cow) => {
      cow.traverse(function (node) {
        if (node.isMesh) node.material = cowmaterial;
      });
      cow.name = "cow";
      cow.position.set(0, 0, 50);
      cow.rotation.y = Math.PI * 0.5;
      cow.scale.set(1, 1, 1);
      scene.add(cow);
    });
    cowobjLoader.load("Objects/Cow/Cow.obj", (cow) => {
